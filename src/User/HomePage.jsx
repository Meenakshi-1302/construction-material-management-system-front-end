...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchMaterials = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/materials/all`);
        setMaterials(response.data);
        const uniqueCategories = Array.from(new Set(response.data.map(material => material.category)));
  ...
  const handleCheckboxChange = (material) => {
    if (compareList.some(item => item.id === material.materialId)) {
      dispatch(removeFromCompare({ id: material.materialId }));
    } else {
      ...
      Swal.fire({
        ...
        text: `${material.materialName} added to compare!`,
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-orange-600 text-white py-4">
        <div className="max-w-8xl mx-auto px-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold">Material Management System</h1>
          <nav className="flex space-x-4">
            <a href="/raw-material-calculator" className="hover:underline">Raw Material Calculator</a>
            <button onClick={() => navigate('/compare')} className="hover:underline">
                ...
                <div key={material.materialId} className="bg-white shadow-lg rounded-lg overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                        checked={compareList.some(item => item.id === material.materialId)}
                        onChange={() => handleCheckboxChange(material)}
                        className="mr-2"
                      />
                      <label className="text-gray-700">Add to Compare</label>
                    </div>
                  </div>
                </div>
              ))}
